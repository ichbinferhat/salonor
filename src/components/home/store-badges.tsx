...
export async function StoreBadges({ className = "" }: { className?: string }) {
  const dict = await getDictionary();
  const comingSoon = dict.common.comingSoon;
  return (
...
function Badge({
  ...
  // Henüz mağazalarda yok: tıklanabilir görünmesin (anchor yok, cursor-default),
  // ekran okuyucu için "yakında" durumu açıkça belirtilsin.
  return (
    <span
      className="flex cursor-default items-center gap-2.5 rounded-2xl bg-gradient-to-b from-ink to-ink-strong px-4 py-2.5 text-white shadow-card ring-1 ring-white/10 transition-all hover:-translate-y-0.5 hover:shadow-pop"
      aria-label={`${store} — ${top}`}
    >
      {glyph}
      <span className="text-left leading-tight" aria-hidden>
