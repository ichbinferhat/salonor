...
export function HoursEditor({ initial }: { initial: DayHour[] }) {
  ...
  function save() {
    startTransition(async () => {
      await updateHoursAction(ORDER.map((wd) => hours[wd]));
      setSaved(true);
    });
  }
  ...
  return (
    <div>
          ...
          return (
            ...
            >
              ...
              <label className="flex cursor-pointer items-center gap-2 text-sm">
                ...
                />
                {h.closed ? <span className="text-ink-mute">Kapalı</span> : <span className="text-ink-soft">Açık</span>}
              </label>
              {!h.closed && (
                <div className="ml-auto flex items-center gap-2">
                  <select
                    value={h.openMin}
                    onChange={(e) => update(wd, { openMin: Number(e.target.value) })}
      ...
      <div className="mt-4 flex items-center gap-3">
        ...
        >
          {isPending ? "Kaydediliyor..." : "Saatleri kaydet"}
        </button>
        {saved && (
