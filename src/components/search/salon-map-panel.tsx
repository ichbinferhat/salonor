  ...
  useEffect(() => {
    ...
    for (const s of salons) {
      ...
      const popup = new maplibregl.Popup({ offset: 14, closeButton: false }).setHTML(
        `<a href="/salon/${s.slug}" style="display:block;padding:4px 2px;text-decoration:none;color:#131b2e;font-family:inherit">
           ...
           <span style="display:block;font-size:12px;color:#5435e6;font-weight:700;margin-top:4px">Salonu görüntüle →</span>
         </a>`
      );
    ...
    <>
      ...
      >
        ...
        {mobileOpen && (
          ...
          >
            <X className="size-4" /> Listeye dön
          </button>
        )}
      </div>
      ...
      {!mobileOpen && (
        ...
        >
          <MapIcon className="size-4" /> Harita
        </button>
      )}
    </>
  );
