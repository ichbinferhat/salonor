...
export default async function AdminPage() {
  const [bizCount, userCount, apptCount, reviewCount, businesses, recentUsers] =
    await Promise.all([
      db.business.count(),
      db.user.count(),
  ...
  const stats = [
    { label: "İşletme", value: bizCount, icon: Building2, tone: "bg-accent-soft text-accent-deep" },
    { label: "Kullanıcı", value: userCount, icon: Users, tone: "bg-mint-soft text-mint" },
    { label: "Randevu", value: apptCount, icon: CalendarDays, tone: "bg-sea-soft text-sea" },
    { label: "Yorum", value: reviewCount, icon: Star, tone: "bg-honey-soft text-honey" },
  ];
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-1 text-ink-soft">Platformdaki tüm işletmeleri ve kullanıcıları yönet.</p>
      </div>
          ...
          <table className="w-full min-w-[720px] text-sm">
            <thead>
              <tr className="border-b border-line text-left text-xs uppercase tracking-wide text-ink-mute">
                ...
                <th className="px-3 py-3 font-bold">Puan</th>
                <th className="px-3 py-3 font-bold">Durum</th>
                <th className="px-5 py-3 font-bold text-right">Sayfa</th>
              </tr>
            </thead>
              ...
              {businesses.map((b) => (
                <tr key={b.id} className="hover:bg-cream/60">
                  ...
                  <td className="px-3 py-3 font-semibold text-ink">
                    {b.ratingAvg.toFixed(1)} <span className="text-ink-mute">({b._count.reviews})</span>
                  </td>
                  <td className="px-3 py-3">
